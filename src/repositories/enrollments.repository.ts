...
function findById(id: string): Promise<Enrollment | null> {
  return prisma.enrollment.findFirst({ where: { id, ...notDeleted } });
}

function findByStudentAndCourse(studentId: string, courseId: string): Promise<Enrollment | null> {
  return prisma.enrollment.findFirst({ where: { studentId, courseId, ...notDeleted } });
}

function remove(id: string): Promise<Enrollment> {
  return prisma.enrollment.update({ where: { id }, data: { deletedAt: new Date() } });
}
...
function count(where: Prisma.EnrollmentWhereInput): Promise<number> {
  return prisma.enrollment.count({ where: { ...where, ...notDeleted } });
}

function findAssignedCourses(studentId: string, skip: number, take: number) {
  return prisma.enrollment.findMany({
    where: { studentId, ...notDeleted },
    include: { course: { include: { _count: { select: { lessons: { where: notDeleted } } } } } },
    orderBy: { createdAt: 'desc' },
    skip,
    take,
  });
...
function countByStudent(studentId: string): Promise<number> {
  return prisma.enrollment.count({ where: { studentId, ...notDeleted } });
}
